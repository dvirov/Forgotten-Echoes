...
#placeholder
execute on target unless items entity @s weapon.mainhand *[custom_data~{id:echo_tuner}] run return fail
execute on target run item modify entity @s weapon.mainhand [{function:"minecraft:set_count",count:-1,add:1b}]
execute on target run advancement grant @s only forgotten_echoes:repair_echo_bell
